...
export function SeriesPage() {
  ...
  const fetchSeries = useCallback(async () => {
    try {
      const res = await fetch('/api/movies?type=series&limit=500');
      if (res.ok) {
  ...
  // Refresh when series are reorganized or scan completes
  useEffect(() => {
    const handler = () => fetchSeries();
    window.addEventListener('series-changed', handler);
    return () => window.removeEventListener('series-changed', handler);
  }, [fetchSeries]);
  ...
  const filteredSeries = useMemo(() => {
    ...
    return result;
  }, [series, selectedGenre, sortBy, searchQuery]);
    ...
    <div className="min-h-[60vh]">
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto mb-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-gray-500 text-sm">{filteredSeries.length} series</span>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          <Tv className="h-12 w-12 text-gray-600 mb-4" />
          <h3 className="text-white text-lg font-medium mb-2">No se encontraron series</h3>
