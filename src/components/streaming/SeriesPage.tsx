  ...
  const genres = useMemo(() => {
    ...
    series.forEach((s) => {
      s.genre.split(',').forEach((g) => {
        ...
        if (trimmed && trimmed !== 'Desconocido') genreSet.add(trimmed);
      });
    });
  ...
  const filteredSeries = useMemo(() => {
    ...
    if (selectedGenre !== 'Todos') {
      result = result.filter((s) =>
        s.genre.split(',').some((g) => g.trim() === selectedGenre)
      );
    }
    ...
    <div className="min-h-[60vh]">
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto mb-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-gray-500 text-sm">{filteredSeries.length} títulos</span>
        </div>
